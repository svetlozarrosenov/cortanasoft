...
export default function AdminContent() {
  ...
  const [newDevice, setNewDevice] = useState({
    name: '', model: '', type: '', description: '', image: '', price: '', serialNumber: ''
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewDevice({ name: '', model: '', type: '', description: '', image: '', price: '', serialNumber: '' });
    } catch (error) {
            ...
            <ul className={styles.deviceList}>
              {products.map((device: any) => (
                <li key={device._id} className={styles.deviceItem}>
                  <div className={styles.deviceInfo}>{device.name} - {device.type}</div>
    ...
    <div className={styles.pageContainer}>
      ...
      <aside className={styles.sidebar}>
        <ul className={styles.sidebarMenu}>
          {['products', 'devices', 'clients', 'settings'].map((tab) => (
            <li key={tab} className={styles.sidebarMenuItem}>
              <Link
                href={buildLink(tab)}
                className={`${styles.sidebarMenuLink} ${activeMenu === tab ? styles.active : ''}`}
              >
                {tab === 'products' && 'Продукти'}
      ...
      <div className={styles.contentContainer}>
        <h1 className={styles.title}>Админ <span>Панел</span></h1>
        {renderContent()}
      </div>
